...
- When the user asks for RCA on a specific broken draft, start with that live `cover_letters` row, its source template, and its related `jobs`/eval rows. Do not widen into generic observability proposals until the concrete row-level trace is exhausted.
