// LinkedIn OAuth service for profile data fetching
...
export class LinkedInOAuthService {
  ...
  /**
   * Get LinkedIn access token from Supabase session
   */
  ...
  /**
   * Fetch LinkedIn profile data using OAuth
   */
  async fetchProfileData(): Promise<LinkedInApiResult> {
    try {
      ...
      if (!this.accessToken) {
        return {
          success: false,
          error: 'No LinkedIn access token available. Please sign in with LinkedIn first.',
          retryable: false
        };
      }
